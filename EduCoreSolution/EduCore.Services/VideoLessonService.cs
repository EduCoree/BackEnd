...
namespace EduCore.Services
{
    public class VideoLessonService : IVideoLessonService
    {
        ...
        public VideoLessonService(IUnitOfWork uow)
        {
            _uow = uow;
        }
            ...
            int lessonId, string studentId, CancellationToken ct)
        {
            ...
            if (video is null)
                throw new NotFoundException("No video attached to this lesson.");

            // 4. Generate expiring signed URL (TTL 2 hours)
            var expiry = DateTime.UtcNow.AddHours(2);
            var signedUrl = GenerateSignedUrl(video.VideoUrl, expiry, studentId);
